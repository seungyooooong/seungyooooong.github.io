...
var portfolio = [{
    ...
    하지만프로젝트를 진행하면서 오히려 기존 경험들 덕분에 WebApp, Web, Native App의 차이를 이해할 수 있었고 생각보다 할만하게 느껴졌습니다.
    ...
    뭐든 꾸준히 하는 것이 제일 어렵다고 중간에 일이 바쁠 때나 휴가철 등에 포스팅이 뜸해지기도 했지만 조금씩이라도 계속 읽어나가면서 언젠가 총 정리를 해내겠다는 목표를 가지고 열심히 읽어나가고 있습니다.`
}, {
    ...
    SwiftUI와 UIKit는 화면을 구성하는 방법부터가 다르다는 것, 내부의 다른 앱으로부터 데이터를 연동하는 방법, 외부 API에서 데이터를 받아오는 법 등 다양한 부분을 재밌게 배울 수 있었습니다.
    뿐만 아니라 Apple에서 제공하는 공식 문서인 만큼 Apple이 추구하는 앱의 접근성이나 코드의 구성에 대해서도 엿볼 수 있어서 좋았습니다.
    Swift의 경우 사용한지 오래되지 않은 언어이다보니 자료가 많이 부족하다고 느껴져 어떻게 공부를 시작해야 할지 고민됐는데, 좋은 자료를 통해 공부를 할 수 있었습니다.`
}]
